...
export async function getMemberInvoices(memberId: string): Promise<MemberInvoiceRow[]> {
  await requireRole(STAFF_ROLES);
  if (!isUuid(memberId)) return [] as MemberInvoiceRow[];
  ...
  return rows as MemberInvoiceRow[];
}
...
export async function getMemberPeriods(memberId: string): Promise<MemberPeriodRow[]> {
  await requireRole(STAFF_ROLES);
  if (!isUuid(memberId)) return [] as MemberPeriodRow[];
  const rows = await getSql()(
  ...
  return rows as MemberPeriodRow[];
}
